...
% Brooke Yeager 
% Last edited by  Brooke on 06/21/2021
% Last did: Changed save output

%% User input %%
UserInput = inputdlg({'Subject Number:','Triggers:','Practice or Testing:','Block Number:', 'Stimulation Condition:'},'Parameters',[1 35;1 35; 1 35; 1 35; 1 35],{'','','','',''}) ; 
...
BlockNum = str2num(UserInput{4}); % 0 = Practice trials; 1 = Working Memory Test trials
StimCond = str2num(UserInput{5}); % 1 = Sham; 2 = Tuned; 3 = Non-tuned
...
if PracOrTest == 0
    ...
    %% Stim type 'if' statement for PRACTICE
        if condition2(i) == 1 % Pulls 3 letter stimuli
            Screen('TextSize',myscreen,120); % Pull letter strings from cell array to display %
    ...
    alphabet1 = {'B' 'C' 'D' 'F' 'G' 'H' 'J' 'K' 'M' 'N' 'P' 'Q' 'R' 'S' 'T' 'V' 'W' 'X' 'Z'}
    ...
    %% Probe trials %%
        ...
        %%% Set stimulus size as 120
        startTime = GetSecs;
        Screen('TextSize',myscreen,120);
        DrawFormattedText(myscreen, lower(probe), 'center','center', [255,255,255]);
        Screen('Flip', myscreen);
...
responseData(:,2) = num2cell(condition); %Column 2: Trial type -- matched = 1, non-matched = 2
...
%% Save data %%
task=['WM_' 'StimType' num2str(StimCond) '_' num2str(SubjNum) '.mat']
