...
% Brooke Yeager 
% Last edited by Brooke on 06/02/2021
...
if PracOrTest == 0
    ...
    %% Pull letter strings from cell array to display %%
    load('PRACTICEStim.mat');
    load('PRACTICEStimCells.mat');
    ...
    Screen('FillRect', myscreen, [255,255,255], FixCross');
    Screen('Flip', myscreen);
    WaitSecs(5) %%5 second  delay it is fixed for all trials, can change this
...
responseData(:,1) = num2cell(1:numTrials) %First column is Trial number
    ...
    WaitSecs(fixationTime)
        ...
        %% Pull letter strings from cell array to display %%
        load('TargStim.mat');
        load('TargStimCells.mat');
        responseData(:,3) = TargStim
